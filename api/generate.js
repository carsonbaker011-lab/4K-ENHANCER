...
Then generate exactly 3 different high-converting Instagram/social media ad image prompts for Nano Banana Pro (an AI image generator). 

Each ad should follow this proven template (like the Alpecin Grey Attack ad style):
  ...
  "ads": [
    {
      ...
      "image_prompt": "Detailed Nano Banana Pro image generation prompt. Be extremely specific about the person, their emotion, the product placement, lighting, background, composition, camera angle, and mood. IMPORTANT: Include large, bold, attention-grabbing typography overlaid on the image — the headline text should be massive, high-contrast, and dominate the composition like a billboard. Use the brand colors for the text. The words should feel like they are punching out of the image. Make it feel like a premium magazine ad. End with: photorealistic, 4K, professional advertising photography, studio lighting, sharp focus, high dynamic range, bold typography, impactful text overlay."
    },
    { ... second ad ... },
    { ... third ad ... }
  ]
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Stagger predictions 3s apart to avoid rate limit burst
    const predictionIds = [];
    for (const ad of brand.ads) {
      const pid = await startNanaBanana(ad.image_prompt);
      predictionIds.push(pid);
