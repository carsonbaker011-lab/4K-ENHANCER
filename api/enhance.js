...
const NANO_BANANA_PROMPT = "Ultra high resolution 8K image enhancement, restore sharp focus and fine details, remove motion blur and softness, enhance facial features with natural skin texture, preserve realistic lighting and colors, improve hair strand definition and clarity, sharpen edges without overprocessing, cinematic depth of field, high dynamic range (HDR), professional photography quality, noise reduction, realistic contrast, natural shadows, crisp details, photorealistic rendering. Do not change the content, composition, or subject of the image in any way — only enhance the resolution, sharpness, and quality of the exact image provided.";
...
async function runClaudeAnalysis(imageBase64, rawMediaType, mode) {
  ...
  const systemPrompt = `Analyze this image and return JSON only, no markdown, no extra text.
{
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log("Claude analysis done, subject:", analysis.subject_type);
    console.log("Blob URL:", imageUrl);

    // Start Nano Banana with Claude's generated prompt
    const pid = await startNanoBanana(imageUrl, NANO_BANANA_PROMPT, resolution || "4K");
