...
async function startStage2(imageUrl, mode) {
  // Stage 2: second ESRGAN pass at higher fidelity + face enhancement
  ...
  const text = await res.text();
  console.log("Stage2 ESRGAN-face:", res.status, text.slice(0, 500));
  if (!res.ok) throw new Error(`Stage2 failed ${res.status}: ${text.slice(0,300)}`);
  const data = JSON.parse(text);
...
async function startStage3(imageUrl) {
  ...
  const text = await res.text();
  console.log("Stage3 Topaz:", res.status, text.slice(0, 500));
  if (!res.ok) throw new Error(`Stage3 failed ${res.status}: ${text.slice(0,300)}`);
  const data = JSON.parse(text);
