...
async function uploadImageToReplicate(imageBase64, mediaType) {
  ...
  if (!res.ok) throw new Error("Failed to upload image to Replicate");
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (count > 1) {
      return res.status(429).json({
        ...
        message: `You've used your 1 free enhancement today. Resets in ~${Math.ceil(ttl / 3600)}h.`,
        resetsIn: Math.ceil(ttl / 3600),
      });
    }
  } catch (e) {
  ...
  try {
    const imageUrl = await uploadImageToReplicate(imageBase64, mediaType);
