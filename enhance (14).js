...
function normalizeMediaType(mediaType) {
  ...
  return map[mediaType?.toLowerCase()] || mediaType || 'image/jpeg';
}

async function runClaudeAnalysis(imageBase64, mediaType, mode) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [analysis, imageUrl] = await Promise.all([
      runClaudeAnalysis(imageBase64, mediaType, mode || "auto"),
      uploadToBlob(imageBase64, mediaType),
    ]);
